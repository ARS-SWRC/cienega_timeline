...
/*
 *
 *  MODIFICATION (Gerardo):
 *  1-15-2014: Added the ability to include sources and tags in the modal windows
 *  1-16-2014: Add the ability to display the tags and creat filters based on the tags
 *  3-30-2022: Customized the font sizing based on importance in compileTickEventsAsHtml
*/
...
tg.TG_TimelinePlayer = function (widget, mediator) {
	...
	*/
	...
	this._templates = {
		...
		+ "{{html image}}<br/><b>Description: </b>{{html description}}<br/><br/>"
		+ "<b>Category: </b> <img src={{html 'timeglider/icons/' + icon}} class='icon_image'>  {{html category}}<br/><br/>"
        + "<b>Sources: </b>{{html source}}<br/><br/>"
        + "<div class='tg-full_modal-links'><ul>{{html links}}</ul></div><br/><br/><br/>"
		+ "<b>Tags: </b>{{html tags}}"
		 + "<div id='insert'></div>"
		+ "</div>"
		...
		+ "</div>"),
     	...
     	filter_modal : $.template( null,
          ...
          "<input placeholder='tags(s)' type='text' id='filter-tags' class='timeglider-filter-tags'>"+
          "</div>"+
           "<div class='timeglider-formline'><a href='#keywords_modal' rel='modal:open'>Show available keywords</a> </div>"+
	...
	$(CONTAINER)
		...
		.delegate(".tg-legend-close", CLICKORTOUCH, function () {
            ...
            // TODO: continue here in order to persist selected legend items after closing legend
	...
	passesFilters : function (ev, zoomLevel) {
		...
		if (MED.filters.include) {
			...
			for (e=0; e<ea.length; e++) {
				...
				//console.log(ei);
				if (ev.title.match(ei) || ev.description.match(ei) || ev.source.match(ei)) { ret = true; }
			}
			...
			if (MED.filters.description) {
                ...
                // cycle through comma separated include keywords
                //console.log(ev);
                //console.log(MED.filters.description)
				for (i=0; i<da.length; i++) {
                    ii = new RegExp($.trim(da[i]), "i");
                    //console.log("Title: " + ev.title);
                    //console.log(ev.description);
					if (ev.description.match(ii) || ev.source.match(ii)) { ret = true; }
				}
			}	
  ...
  compileTickEventsAsHtml : function (tl, idArr, tick_serial, btype, tickUnit) {
		...
		// BBONE
      	...
      	if (this.passesFilters(ev, zl) === true) {
      		...
      		if (expCol == "expanded") {
                ...
                Font_Size = MED.base_font_size * impq;
                // Gerardo: adjust importance size for 80 and 90
                if (ev.importance == 100){
                    Font_Size = Font_Size * 0.70;
                }
                else if (ev.importance == 80){
                    Font_Size = Font_Size * 0.80;
                }
                else if (ev.importance == 40){
	...
	*/  
	...
	eventModal : function (eid, $event) {
        ...
        // Gerardo's note: the tags object is undifined at this point, not loaded from jSON file
        // ev2 = MED.eventCollection.get(eid).attributes;
        //console.log("DESCRIPTION: " + ev2.description);
        //console.log("TAGS: " + ev2.tags);
